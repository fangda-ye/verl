"""
Example reward functions for GroupAwareRewardManager.

These examples demonstrate how to implement adaptive reward scoring that considers
all responses generated for the same prompt when computing rewards.

This is useful for GRPO and other algorithms where multiple responses are sampled
per prompt, and you want to reward responses based on their relative performance
within the group.
"""
...
def proactive_group_aware_reward(
    ...
    **kwargs,
    ...
    """
    Proactive group-aware reward function.
    
    Scoring rules:
    1. Correctness: +1 if ground truth in response, else 0
    ...
    for response, gt in zip(responses, ground_truths):
        is_correct = gt.strip().lower() in response.strip().lower()
...
def adaptive_difficulty_reward(
    ...
    **kwargs,
    ...
    for i, response in enumerate(responses):
        ground_truth = ground_truths[i]
        correct = ground_truth.strip().lower() in response.strip().lower()
...
def relative_quality_reward(
    ...
    **kwargs,
    ...
    for i, response in enumerate(responses):
        ...
        has_answer = ground_truth.strip().lower() in response.strip().lower()
        length_score = min(len(response) / 100.0, 1.0)  # Normalize length
